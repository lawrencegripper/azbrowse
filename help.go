...
func ToggleHelpView(g *gocui.Gui) {
	...
	if showHelp {
		g.Update(func(g *gocui.Gui) error {
			...
			maxX = maxX - 2
			maxY = maxY - 2
			v, err := g.SetView("helpWidget", 1, 1, maxX-1, maxY-1)
...
func DrawHelp(v *gocui.View) {
	...
	--> PRESS CTRL+H TO CLOSE THIS VIEW AND CONTINUE. YOU CAN OPEN IT AGAIN WITH CRTL+H AT ANY TIME. <--                                                                                             
                             _       ___                                                                                                                                                            
...
`)
	...
	fmt.Fprint(v, style.Header(` ## Navigation `))
...
`)
	...
	fmt.Fprint(v, style.Header(`## Operations`))
...
`)
	fmt.Fprint(v, style.Header(`--> PRESS CTRL+H TO CLOSE THIS VIEW AND CONTINUE. YOU CAN OPEN IT AGAIN WITH CRTL+H AT ANY TIME. <-- `))
